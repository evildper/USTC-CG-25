...
Color Hd_USTC_CG_Rect_Light::Sample(
    ...
    const std::function<float()>& uniform_float)
{
    auto worldSampledDir = uniform_float() * width * (corner1 - corner0) +
                           uniform_float() * height * (corner2 - corner0);

    auto sampledPos = worldSampledDir + corner0;
    sampled_light_pos = sampledPos;

    // Then we can decide the direction.
    auto distanceVec = sampledPos - pos;
    auto distance = distanceVec.GetLength();
    dir = distanceVec.GetNormalized();

    // and the pdf (with the measure of solid angle):
    float cosVal = GfDot(-dir, normal);

    sample_light_pdf = 1 / width / height / cosVal * distance * distance;
...
Color Hd_USTC_CG_Rect_Light::Intersect(const GfRay& ray, float& depth)
{
    double distance;
    bool frontFacing;
    if ((ray.Intersect(corner0, corner1, corner2, &distance, NULL, &frontFacing) && frontFacing) ||
        (ray.Intersect(corner2, corner1, corner3, &distance, NULL, &frontFacing) && frontFacing)) 
    {
        depth = distance;
        return irradiance;
    }
    ...
    HdDirtyBits* dirtyBits)
{
    ...
    normal = GfCross(corner1 - corner0, corner2 - corner0).GetNormalized();
}
