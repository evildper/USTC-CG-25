...
void SeamlessClone::set_images(std::shared_ptr<Image> src, std::shared_ptr<Image> tar, std::shared_ptr<Image> mask) {
    src_img_ = src;
    tar_img_ = tar;
    src_selected_mask_ = mask;
}
void SeamlessClone::set_right_to_mouse(int a,int b){
    right_to_mouse_x = a;
    right_to_mouse_y = b;
}
void SeamlessClone::compute_bounding_box() {
...
std::shared_ptr<Image> SeamlessClone::solve() {
    ...
    for (auto [flat_idx, local_idx] : mask_index_map) {
        ...
        if(mask_index_map.count((y - 1) * W + x)&&mask_index_map.count((y + 1) * W + x)&&mask_index_map.count(y * W + (x - 1))&&mask_index_map.count(y * W + (x + 1))){
            ...
            for(int c = 0;c < 3;c++)
            {
                B_channels[c](local_idx) = 4*src_img_->get_pixel(global_x,global_y)[c]-src_img_->get_pixel(global_x+1,global_y)[c]-src_img_->get_pixel(global_x-1,global_y)[c]-src_img_->get_pixel(global_x,global_y+1)[c]-src_img_->get_pixel(global_x,global_y-1)[c];
            }
        }
        else{
            for(int c=0;c<3;c++)
...
void SeamlessClone::precompute_A() {
    compute_bounding_box();
    int W = width_, H = height_;
    auto is_boundary = [&](int x, int y) {
        if (src_selected_mask_->get_pixel(x, y)[0] == 0) return false; // 不是掩码区域
...
bool SeamlessClone::judge_A_precomputed() {
...
// void SeamlessClone::set_size(int width, int height) {
//     width_ = width;
//     height_ = height;
// }

// void SeamlessClone::set_offset(int x, int y) {
//     offset_x_ = x;
//     offset_y_ = y;
// }
...
//                 for (int c = 0; c < 3; ++c) {
//                     int idx = y * W + x;
//                     B_channels[c](idx) = 4.0 * src_img_->get_pixel(x+p.first,y+p.second)[c] - src_img_->get_pixel(x+p.first+1,y+p.second)[c] - src_img_->get_pixel(x-1+p.first,y+p.second)[c] -src_img_->get_pixel(x+p.first,y+1+p.second)[c] - src_img_->get_pixel(x+p.first,y-1+p.second)[c];
//                 }
//             }
