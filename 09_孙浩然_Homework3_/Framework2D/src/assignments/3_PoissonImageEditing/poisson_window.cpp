...
void PoissonWindow::draw_toolbar()
{
    if (ImGui::BeginMainMenuBar())
    {
        ...
        // // **当 Select 选中时，显示 Rect/Polygon 选项**
        // if (selectable) {
        //     static int selected_region_type = 0; // 0 = Rect, 1 = Polygon
        //     ImGui::Text("Region Type:");
        //     if (ImGui::Combo("##RegionType", &selected_region_type, "Rect\0Polygon\0\0")) {
        //         if (p_source_) {
        //             p_source_->set_region_type(static_cast<USTC_CG::SourceImageWidget::RegionType>(selected_region_type + 1));
        //         }
        //     }
        // }
        ...
        ImGui::Separator();
        if(ImGui::MenuItem("Polygon")&&p_source_&&p_target_)
        {       
            
            int a = 1;
            p_source_->set_region_type(a);
        }
        ImGui::Separator();
        if(ImGui::MenuItem("Rect")&&p_source_&&p_target_)
        {       
        ...
        add_tooltips(
            "Press this button and then click in the target image, to "
            "clone the selected region to the target image with mixing.");
