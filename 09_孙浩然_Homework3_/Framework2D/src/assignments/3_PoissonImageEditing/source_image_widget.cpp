...
void SourceImageWidget::set_region_type(int a)
{
    if(a==1)
...
void SourceImageWidget::mouse_click_event()
{
    ...
    if(region_type_== kPolygon){
        if(!draw_status_){
            draw_status_=true;
            start_= mouse_pos_in_canvas();
            selected_shape_  = std::make_unique<Polygon>(start_.x,start_.y); 
            ...
            // }
            ImVec2 tmp = mouse_pos_in_canvas();
            selected_shape_ ->add_control_point(tmp.x,tmp.y);
